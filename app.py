...
# Премахване на дублиращи се записи в Fi данните
Fi_data = Fi_data.drop_duplicates(subset=['x', 'Fi'])
...
for fi in fi_values_available:
    ...
    if len(group) == 1:
        ...
        # Ако има повече от една точка, създаваме интерполатор
...
for h in h_values_available:
    # Намираме средната x стойност за всяко H
    x_val = H_data[H_data['H'] == h]['x'].mean()
    h_to_x[h] = x_val
...
for fi_val in fi_values_available:
    ...
    if len(group) == 1:
        ...
        x_min = Fi_data['x'].min()
        x_max = Fi_data['x'].max()
        y_const = group['y'].iloc[0]
        if fi_val == fi_value:
            ax.plot([x_min, x_max], [y_const, y_const], 'b-', linewidth=3, label=f'Fi={fi_val} (избрана)')
        else:
            ax.plot([x_min, x_max], [y_const, y_const], 'b-', linewidth=1, alpha=0.3)
    else:
        ...
        if fi_val == fi_value:
            ...
            ax.scatter(group['x'], group['y'], color='blue', s=50, alpha=0.7)
        else:
...
st.write(f"Брой точки за Fi: {len(Fi_data)}, Брой точки за H: {len(H_data)}")
